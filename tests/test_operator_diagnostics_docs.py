...
def test_post_v019_plan_is_active_without_expanding_scope():
    ...
    for expected in (
        ...
        "Stage status: B - baseline reconciliation docs/tests are implemented",
        "`v0.1.9` is published at https://github.com/YSCJRH/WinChronicle/releases/tag/v0.1.9",
        "targets `d06ab5bc8bea7520bac2719adb457794c72911d3`",
...
def test_release_evidence_freshness_guard_labels_inherited_manual_smoke():
    ...
    for expected in (
        ...
        "active post-v0.1.8 compatible maintenance path toward `v0.1.9`",
...
def test_manual_smoke_ledger_tracks_freshness_without_observed_artifacts():
    ...
    for expected in (
        ...
        "Last freshness decision | For the post-v0.1.8 compatible maintenance path that published `v0.1.9`",
        "inherited `v0.1.0` Notepad, Edge, VS Code metadata",
        "was explicitly accepted by the W4 release-readiness record",
        ...
        "Next freshness decision | X1 release evidence freshness must explicitly accept inherited manual evidence",
        ...
        "stale/inherited until X1 makes a release-specific freshness decision",
