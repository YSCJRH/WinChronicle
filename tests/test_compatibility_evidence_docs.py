...
MEMORY_SCORECARD = ROOT / "harness" / "scorecards" / "memory-quality.md"
...
def test_scorecards_remain_the_evidence_oracles_for_mcp_and_phase6():
    ...
    memory = MEMORY_SCORECARD.read_text(encoding="utf-8")
    ...
    assert "MCP output is local evidence, not permission to publish or share results." in mcp
    assert "External sharing still requires explicit user approval." in mcp
    assert "Memory output is local evidence, not permission to publish or share results." in memory
    assert "External sharing still requires explicit user approval." in memory
...
def test_v0118_release_record_is_published_and_scoped():
    text = V0118_RELEASE.read_text(encoding="utf-8")
    normalized = _normalized(text)

    for phrase in (
        "`v0.1.18` is published as a compatible maintenance release.",
        "Publication status: published maintenance release.",
        "Release | `v0.1.18`",
        ...
        "Candidate PR Windows Harness | Passed, run `25612336939`",
        "Candidate post-merge `main` Windows Harness | Passed, run `25612391276`",
        "`gh release view v0.1.18 --json tagName,name,url,targetCommitish,isDraft,isPrerelease,publishedAt` before publication",
        "release not found, confirming no existing `v0.1.18` release to retag",
        "`git tag --list \"v0.1.18*\"` before publication | Pass | no local tags returned",
        "`git ls-remote --tags origin v0.1.18 v0.1.18-rc.0` before publication | Pass | no remote tags returned",
        "`gh release create v0.1.18 --target 2e22ec9805edb0efd48e5ef4aacbcff13f0490ec` | Pass",
        "`gh release view v0.1.18 --json tagName,name,url,targetCommitish,isDraft,isPrerelease,publishedAt` | Pass | `v0.1.18` is published",
        ...
        "Publication approval: standing user goal authorizes publishing after review",
        "GitHub release publication: passed.",
        "This publication reconciliation records those post-publication facts",
        "Release URL: https://github.com/YSCJRH/WinChronicle/releases/tag/v0.1.18.",
        "Final tag target: `2e22ec9805edb0efd48e5ef4aacbcff13f0490ec`.",
    ):
        assert phrase in normalized
    ...
    for stale_phrase in (
        "this publication reconciliation.",
        "ready for PR review as a compatible maintenance release candidate",
...
def test_v0119_release_record_is_published_and_scoped():
    text = V0119_RELEASE.read_text(encoding="utf-8")
    normalized = _normalized(text)

    for phrase in (
        "`v0.1.19` is published as a compatible maintenance release.",
        "Publication completed after local deterministic validation, PR review, PR Windows Harness, post-merge `main` Windows Harness",
        "Publication status: published maintenance release.",
        "Release | `v0.1.19`",
        ...
        "Publication reconciliation PR Windows Harness | Passed, run `25897590925`",
        ...
        "Candidate PR Windows Harness | Passed, run `25896736903`",
        "Candidate post-merge `main` Windows Harness | Passed, run `25896975136`",
        ...
        "`gh release create v0.1.19 --target c087f9e5daaf9e48b5529b5f7188d047714f3552` | Pass",
        "`gh release view v0.1.19 --json tagName,name,url,targetCommitish,isDraft,isPrerelease,publishedAt` | Pass | `v0.1.19` is published",
        ...
        "completed after local deterministic validation, PR review, PR Windows Harness",
        "Publication approval: standing user goal authorizes publishing after review",
        "GitHub release publication: passed.",
        "Release URL: https://github.com/YSCJRH/WinChronicle/releases/tag/v0.1.19.",
        "Final tag target: `c087f9e5daaf9e48b5529b5f7188d047714f3552`.",
    ...
    for stale_phrase in (
        "Publication status: local deterministic validation passed; pending",
        "Release URL | Pending:",
        "GitHub release publication: pending.",
...
def test_post_v0118_plan_is_active_without_expanding_scope():
    ...
    for phrase in (
        ...
        "previous maintenance loop is closed for human product review",
        "Last completed evidence: AH17 `v0.1.19` release-readiness PR #206 merged",
        "PR Windows Harness run `25896736903` passed",
        "post-merge `main` Windows Harness run `25896975136` passed",
        "GitHub release `v0.1.19` was published at `2026-05-15T02:31:50Z`",
        "AH18 publication reconciliation PR #207 merged",
        "verified the post-AH17 `main` Windows Harness concluded `success`",
        "verified the AH18 publication reconciliation PR Windows Harness concluded `success`",
        "verified the post-AH18 `main` Windows Harness concluded `success`",
        ...
        "These stages are preserved as history. They are not an automatically authorized backlog.",
        ...
        "gh pr view 189",
        "PR #189 merged as `f4d24adf5bb60cd5ad6abfc21ada04fbbeae288c`",
        "gh run view 25613203047",
        "post-AH0 `main` Windows Harness concluded `success`",
        "Stage AH1 initialization:",
        "gh repo view YSCJRH/WinChronicle",
        "repository is public on `main`, is not archived, forked, or private",
        "reports latest release `v0.1.18`",
        "has no custom OpenGraph image",
        ...
        "gh run view 25614929381",
        "post-AH5 `main` Windows Harness concluded `success`",
        ...
        "gh pr view 197",
        "PR #197 merged at `2026-05-10T00:58:38Z`",
        "gh run view 25616023224",
        "post-AH8 `main` Windows Harness concluded `success`",
        "Stage AH9 initialization:",
        "Reviewed `docs/next-round-plan-post-v0.1.18.md`",
        "Found AH8 changed tests/docs/scorecards only",
        "Selected fixture/helper privacy index parity",
        "Stage AH9 local validation:",
        "Read-only reviewer completed before final validation",
        "stale public index labels and missing AH8 release/evidence-index merge/run evidence were addressed",
        "passed, 93 tests",
        "passed, 218 tests",
        "git diff --check",
        "printed no files, confirming AH9 does not change version metadata",
        "stale AH8/current-watcher wording scan",
        "python harness/scripts/run_harness.py",
        "including 218 pytest tests",
        "Stage AH9 completion:",
        "gh pr view 198",
        "PR #198 merged at `2026-05-10T01:15:20Z`",
        "gh run view 25616305777",
        "post-AH9 `main` Windows Harness concluded `success`",
        "Stage AH10 initialization:",
        "direct fixtures, helper output, and watcher captures converge on normalization",
        "Added `tests/test_privacy_index_parity.py`",
        "Stage AH10 local validation:",
        "passed, 2 tests",
        "passed, 7 tests",
        "passed, 94 tests",
        "passed, 221 tests",
        "printed no files, confirming AH10 does not change version metadata",
        "stale AH9/current-follow-up wording scan",
        "including 221 pytest tests",
        "Stage AH10 completion:",
        "gh pr view 199",
        "PR #199 merged at `2026-05-10T01:33:45Z`",
        "gh run view 25616618385",
        "post-AH10 `main` Windows Harness concluded `success`",
        "Stage AH11 initialization:",
        "Found AH10 changed tests/docs/scorecards only",
        "Selected fixture/privacy parity matrix consolidation",
        "Stage AH11 local validation:",
        "Read-only reviewer completed before final validation",
        "stale AH10 next-step wording",
        "passed, 94 tests",
        "passed, 221 tests",
        ...
        "gh pr view 201",
        "PR #201 merged at `2026-05-10T02:10:33Z`",
        "gh run view 25617277557",
        "post-AH12 `main` Windows Harness concluded `success`",
        "Stage AH13 initialization:",
        "Found AH12 changed docs/tests/scorecards only",
        ...
        "PR #203 merged at `2026-05-10T02:48:11Z`",
        "gh run view 25617962810",
        "post-AH14 `main` Windows Harness concluded `success`",
        "Stage AH15 initialization:",
        "Found AH14 changed product privacy output and redaction behavior narrowly",
        ...
        "PR #206 merged at `2026-05-15T02:28:42Z`",
        "gh run view 25896736903",
        "post-AH17 `main` Windows Harness concluded `success`",
        "gh release create v0.1.19",
        "`v0.1.19` is published, not a draft, not a prerelease",
        "git ls-remote --tags origin v0.1.19",
        "Stage AH18 initialization:",
        "AH18 is docs/tests-only",
        "Validated AH18 publication reconciliation PR #207 with Windows Harness run",
        "Stage AH18 local validation:",
        "passed, 98 tests",
        "printed no files, confirming AH18 is docs/tests only",
        "stale current `v0.1.19` pending-publication wording scan",
    ):
        assert phrase in normalized
...
def test_post_v0117_plan_is_active_without_expanding_scope():
    ...
    for phrase in (
        ...
        "release tag targets `2e22ec9805edb0efd48e5ef4aacbcff13f0490ec`",
        "published at `2026-05-09T21:38:33Z`",
        "Current stage: v0.1.18 Publication Reconciliation.",
        "Stage status: `v0.1.18` is published; this branch reconciles mainline",
        "Last completed evidence: v0.1.18 release-readiness PR #187 merged as",
        "PR Windows Harness run `25612336939` passed",
        "post-merge `main` Windows Harness run `25612391276`",
        "release metadata/tag verification passed for `v0.1.18`",
        "verified the post-PR #186 `main` Windows Harness concluded `success`",
        "was limited to `src/winchronicle/capture.py`",
        "bumped version identity to `0.1.18`",
        "kept publication pending until PR review",
        ...
        "Post-publication verification confirmed `v0.1.18` is published",
        "remote tag verification matched that SHA",
        "Next atomic task: land the `v0.1.18` publication reconciliation PR",
        ...
        "v0.1.18 Publication Reconciliation",
        ...
        "PR #177 merged at `2026-05-09T18:31:02Z`",
        "PR #177 Windows Harness concluded `success`",
        "post-reconciliation `main` Windows Harness concluded `success`",
        "Phase 6 privacy-enrichment residual schema coverage audit local validation:",
        "passed, 90 tests",
        "passed, 187 tests",
        "Completed the Phase 6 residual schema coverage audit",
        "Selected the next Phase 6 step as a residual policy fixture expansion",
        "Phase 6 privacy-enrichment residual schema coverage audit completion:",
        "PR #178 merged at `2026-05-09T18:47:17Z`",
        "PR #178 Windows Harness concluded `success`",
        "post-residual-audit `main` Windows Harness concluded `success`",
        "Started the Phase 6 residual policy fixture expansion",
        ...
        "PR #179 merged at `2026-05-09T19:03:47Z`",
        "PR #179 Windows Harness concluded `success`",
        "post-residual-policy-fixtures `main` Windows Harness concluded `success`",
        "Selected the next Phase 6 step as a deferred fixture coverage decision",
        "Phase 6 privacy-enrichment residual policy evidence reconciliation completion:",
        "PR #180 merged at `2026-05-09T19:13:41Z`",
        "PR #180 Windows Harness concluded `success`",
        "post-reconciliation `main` Windows Harness concluded `success`",
        "Started the Phase 6 deferred fixture closure",
        ...
        "PR #181 merged at `2026-05-09T19:33:51Z`",
        "PR #181 Windows Harness concluded `success`",
        "post-deferred-fixture-closure `main` Windows Harness concluded `success`",
        "Selected the next Phase 6 step as a contract closure release-readiness decision",
        "Phase 6 privacy-enrichment deferred fixture closure reconciliation completion:",
        "PR #182 merged at `2026-05-09T19:44:58Z`",
        "PR #182 Windows Harness concluded `success`",
        "post-reconciliation `main` Windows Harness concluded `success`",
        ...
        "Completed the privacy-policy contract parity audit",
        ...
        "Completed `v0.1.18` release-readiness and publication",
        "release metadata/tag verification passed",
        "Started the `v0.1.18` publication reconciliation",
...
def test_phase6_contract_closure_release_decision_declines_publication_path():
    text = PHASE6_CONTRACT_CLOSURE_RELEASE_DECISION.read_text(encoding="utf-8")
    normalized = _normalized(text)

    for phrase in (
        "Phase 6 Contract Closure Release-Readiness Decision After v0.1.17",
        "no: do not start a release-readiness or publication path from these contract-only changes alone",
        "Do not retag `v0.1.17`",
        "did not change runtime code, package version identity, helper/watcher binaries, CLI/MCP output contracts",
        "Is a release-readiness path warranted? | No.",
        "Is immediate publication warranted? | No.",
        "Should `v0.1.17` be retagged? | No.",
        "Should a new version target be selected here? | No.",
        "Is fresh manual UIA smoke decided here? | No.",
        "Select the next blueprint lane with contracts, fixtures, tests, and scorecards first",
        "`harness/specs`, `harness/fixtures/phase6`, `harness/scorecards` | Added the Phase 6 privacy-enrichment spec-only contract",
        "`src/winchronicle`, `resources`, `pyproject.toml` | No diff from the published `v0.1.17` tag",
        "Latest published release remains https://github.com/YSCJRH/WinChronicle/releases/tag/v0.1.17",
        "`v0.1.17` is not a draft or prerelease",
        "Phase 6 deferred fixture closure PR #181 merged as `21b8da7d1dc5e84e8c3a49e3e6e852644ce06830`",
        "PR #181 Windows Harness run `25609874695` concluded `success`",
        "Post-deferred-fixture-closure `main` Windows Harness run `25609934759` concluded `success`",
        "Phase 6 deferred fixture closure reconciliation PR #182 merged as `1675dce9378efb55a226ebe8ac4929a8b196bb04`",
        "PR #182 Windows Harness run `25610106205` concluded `success`",
        "Post-reconciliation `main` Windows Harness run `25610156997` concluded `success`",
        ...
        "focused Phase 6/docs validation reported 93 tests",
        "full pytest reported 190 tests",
        "product-source contract-artifact reference scan returned no matches",
        "full deterministic harness passed, including 190 pytest tests",
        "does not authorize implementation of screenshot capture",
        "privacy-positive and runtime-neutral",
        "Observed content remains untrusted",
        "Do not implement screenshot capture, OCR, raw screenshot caches, runtime allowlist parsing",
    ):
        assert phrase in normalized
...
def test_watcher_privacy_fixture_parity_post_v018_records_temp_fixture_scope():
    ...
    for phrase in (
        ...
        "PR #197 merged at `2026-05-10T00:58:38Z`",
        "PR #197 Windows Harness run `25616023224` concluded `success`",
        "Post-AH8 `main` Windows Harness run `25616063920` concluded `success`",
...
def test_fixture_helper_privacy_index_parity_post_v018_records_direct_paths():
    text = FIXTURE_HELPER_PRIVACY_INDEX_PARITY_V0118.read_text(encoding="utf-8")
    normalized = _normalized(text)

    for phrase in (
        "Fixture/Helper Privacy Index Parity After v0.1.18",
        "AH8 proved watcher-dispatched privacy parity",
        "direct fixture captures and synthesized UIA helper captures",
        "existing synthetic privacy fixtures",
        "password_field.json",
        ...
        "source = \"uia_helper\"",
        "trigger.source = \"win_uia_helper\"",
        "API key, GitHub token, Slack token, JWT, and private key redaction",
        "passed, 2 tests",
        "passed, 7 tests",
        "passed, 94 tests",
        "passed, 221 tests",
...
def test_privacy_fixture_parity_matrix_post_v018_consolidates_paths():
    ...
    for phrase in (
        ...
        "focused docs/matrix validation reported 100 tests",
        "follow-up docs/privacy validation reported 106 tests",
        "full pytest reported 223 tests",
        "stale AH11/AH10 fixture-helper wording scan returned no matches",
        "full deterministic harness passed",
        "PR #201 merged at `2026-05-10T02:10:33Z`",
        "PR #201 Windows Harness run `25617277557` concluded `success`",
...
def test_privacy_policy_contract_parity_audit_records_runtime_hardening():
    text = PRIVACY_POLICY_PARITY_AUDIT.read_text(encoding="utf-8")
    normalized = _normalized(text)

    for phrase in (
        "Privacy Policy Contract Parity Audit After v0.1.17",
        "The audit found one privacy-check validation gap",
        "plain WinChronicle token canaries have independent regression coverage",
        "Next blueprint lane selection PR #184 merged as `998403d739570dd81677d4f8b3b8244b8a769caf`",
        "PR #184 Windows Harness run `25610819243` concluded `success`",
        "Post-merge `main` Windows Harness run `25610880531` concluded `success`",
        "Disabled capture and control surfaces",
        "App denylist",
        "Title denylist",
        "Redaction rules",
        "Plain token canary evidence",
        "Existing normalized denylisted captures",
        "Existing normalized password fields",
        ...
        "full pytest reported 201 tests",
        "working-tree runtime diff was limited to `src/winchronicle/capture.py`",
        "full deterministic harness passed, including 201 pytest tests",
        "does not authorize screenshot capture",
        "product targeted capture",
        "MCP write tools",
