...
| Raw term absence across storage and search | Direct fixture parity asserts raw terms are absent from capture-buffer JSON, memory Markdown, SQLite `captures`, `captures_fts`, `entries`, `entries_fts`, `search_captures`, `search_memory_entries`, and full MCP search payloads with redacted query echoes. | Synthesized helper parity asserts the same absence surfaces as direct fixture parity. | Watcher parity asserts raw terms are absent from capture files, memory files, capture search, memory search, and MCP memory search, and keeps watcher JSONL out of `WINCHRONICLE_HOME`. | Hard automated privacy gate. |
...
| Denylist skip-before-storage | `tests/test_fixture_capture.py::test_denylisted_app_capture_is_skipped` covers direct denylisted fixture skip behavior. | `tests/test_privacy_index_parity.py::test_uia_helper_denylisted_privacy_captures_are_skipped_without_artifacts` covers denylisted app and title-denylisted helper records with no capture files, memory files, SQLite captures, or searchable observed text. | Watcher parity asserts denylisted app and title-denylisted watcher events increment `denylisted_skipped`, write no capture artifact, and leave no searchable observed text. | Hard privacy gate. |
...
AH14 later strengthened this matrix by adding direct helper denylist evidence
and redacting secret-like MCP search query echoes. The matrix still does not add
a new capture surface, does not store password fields or obvious secrets, does
not commit raw helper or watcher streams, does not commit generated captures or
memory, and does not change live UIA, watcher, screenshot, OCR, clipboard,
keyboard, audio, network, LLM, or desktop-control behavior.
