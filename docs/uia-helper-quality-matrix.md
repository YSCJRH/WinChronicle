...
This matrix is the post-v0.1 helper-quality contract for maintenance work. It
summarizes the current helper coverage without expanding the product capture
...
| Harness-only targeted smoke | Notepad | Unique text marker appears in captured visible/focused text; target metadata shows harness-only HWND capture; no activation or desktop control. | Latest full manual `v0.2.0` release: pass; see `docs/release-v0.2.0.md` and `docs/manual-smoke-evidence-ledger.md`. | Local JSON artifact only; record path in manual smoke evidence, do not commit observed content. | Medium; validates simple edit text while preserving no-store artifact policy. | Hard manual release gate. |
| Harness-only targeted smoke | Microsoft Edge | Local HTML body marker appears in browser text; process/title target is harness-only; URL extraction remains best-effort. | Latest full manual `v0.2.0` release: pass; see `docs/release-v0.2.0.md` and `docs/manual-smoke-evidence-ledger.md`. | Local JSON artifact only; record path, do not commit local page contents or capture JSON. | Medium; browser text may contain observed content. | Hard manual release gate. |
| Harness-only targeted smoke | VS Code metadata | `code.cmd` is available, capture resolves VS Code/Code.exe window metadata, and target metadata shows harness-only process/title capture. | Latest full manual `v0.2.0` release: pass with diagnostic warning; `code.cmd` was available. | Local JSON artifact only; record path, do not commit editor contents. | Medium; metadata confirms target resolution while avoiding dependence on editor buffer text. | Conditional hard manual release gate when `code.cmd` is available. |
| Harness-only targeted smoke | VS Code strict Monaco marker | Editor marker appears through standard UIA text/value extraction. | Latest full manual `v0.2.0` release: diagnostic failure, consistent with known Monaco/UIA limitation. | Keep diagnostic artifact locally and record path only. | Medium; failure must not trigger screenshot/OCR/keyboard/clipboard/control workarounds. | Diagnostic, non-blocking for v0.2. |
| Manual frontmost smoke | Operator-selected foreground app | Product-shaped `capture-frontmost` captures only the foreground window through caller-provided helper path. | Frontmost-only diagnostic last recorded `v0.1.0` final: direct frontmost smoke returned `SKIPPED: helper returned no capture` in an agent-hosted desktop, so this remains diagnostic. | Temporary state only; do not print or commit observed content. | Medium; validates product path without targeted capture. | Diagnostic/manual confidence gate. |
...
For compatible maintenance releases after `v0.1.9`, use this matrix together
...
coverage starts diagnostic until it has fixtures, privacy review, manual smoke
evidence, and an explicit scorecard update.
