...
This AG2 sweep reviews helper and watcher preview diagnostics after the
published `v0.1.17` maintenance release, AG1 public metadata audit, and AG1
...
AG2 found no new helper/watcher diagnostics drift after `v0.1.17`. The reviewed
...
non-persistence, and product targeted-capture pass-through rejection.

No schema, successful CLI/MCP JSON, helper/watcher capture behavior, privacy
storage behavior, or capture-surface change is warranted. AG2 therefore
refreshes documentation/tests only and keeps the AF2 content-free diagnostic fix
as historical evidence.

Fresh manual UIA smoke remains outside default CI. The AF6 manual UIA smoke is
fresh for the published `v0.1.17` maintenance release record, but future
...
The next smallest implementation task is to land this AG2 review through PR
...
- Stale AG0/AG1 cursor and v0.1.16 helper/watcher typo scan across `README.md`,
  `docs`, and `tests` - passed with no matches.
- `python harness/scripts/run_harness.py` - passed, including 172 pytest tests,
