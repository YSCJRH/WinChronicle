...
This audit records the AD1 public metadata and evidence freshness review after
the published `v0.1.15` baseline. It records current evidence and maintainer
...
```powershell
gh release view v0.1.15 --json tagName,url,targetCommitish,isDraft,isPrerelease,publishedAt,name
```
...
Post-AD0 main gate check:

```powershell
gh run view 25593607384 --json databaseId,status,conclusion,headSha,url,displayTitle
```

| Field | Value |
| --- | --- |
| Run | `25593607384` |
| Display title | `Add post-v0.1.15 maintenance cursor (#135)` |
| Head SHA | `90fff5cc25b770634c92669e70c4067b58a8a6ea` |
| Conclusion | `success` |
| URL | https://github.com/YSCJRH/WinChronicle/actions/runs/25593607384 |
...
| Operator entry | `docs/operator-quickstart.md` links release checklist, evidence, smoke gates, roadmap, known limitations, and the active post-v0.1.15 plan. | Operator entry is current. |
| Release evidence freshness | `docs/release-checklist.md`, `docs/release-evidence.md`, and `docs/manual-smoke-evidence-ledger.md` distinguish active post-v0.1.15 evidence from inherited manual UIA smoke. | Evidence freshness is explicit. |
...
AD1 finds no required product-code change. The only public metadata gaps are
manual repository settings: description, topics, optional homepage, and social
preview verification. The next smallest implementation task is AD2: review
