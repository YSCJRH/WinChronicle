...
This sweep records the AB4 compatibility check for the post-v0.1.13 maintenance
...
text, deterministic fixture/golden content, explicit forbidden-term tests, and
the local MCP smoke request variable name. No new runtime dependency or
implementation path was found.

## Decision

AB4 found no compatibility drift and no required product-code or schema change.
The next smallest implementation task is AB5 release readiness for the
compatible `v0.1.14` maintenance target, with publication still gated on local,
