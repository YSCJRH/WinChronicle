...
This AC3 sweep reviews read-only MCP examples, durable memory docs, and the
deterministic demo after the published `v0.1.14` baseline. It records current
...
| Deterministic tests | `tests/test_mcp_tools.py`, `tests/test_memory_pipeline.py`, `tests/test_compatibility_contracts.py`, `tests/test_state_compatibility.py`, and harness smoke scripts cover exact tools, memory search, idempotence, empty state, and trust metadata. | No missing deterministic coverage was found for the AC3 contract list. |
...
AC3 found no required MCP or memory product-code change. The reviewed docs,
...
No fresh manual UIA smoke is required for this AC3 sweep because it changes
...
The next smallest implementation task is AC4: re-run compatibility guardrails
