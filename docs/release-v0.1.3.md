# v0.1.3 Maintenance Release Readiness Record

This record prepares the compatible `v0.1.3` maintenance release from the
...
`v0.1.3` is prepared as a compatible maintenance release. It must not be
published until explicit publication approval is granted.
...
publication, do not publish `v0.1.3`. Prepare a release candidate or a new
maintenance readiness branch instead.
...
| --- | --- |
| Release | `v0.1.3` |
| Stage | Maintenance release readiness |
...
| M4 post-merge `main` Windows Harness | Pending |
...
The M4 PR Windows Harness passed. The post-merge `main` Windows Harness must
pass before publication.

## Release Notes

- Aligns package, runtime, and MCP server version identity to `0.1.3`.
- Records `v0.1.3` release-readiness evidence from the post-v0.1.2 maintenance
  round.
...
- Phase 6 remains specification-only; this maintenance readiness branch
  introduces no screenshot capture code, no OCR engine integration, no
  screenshot cache, no cache cleanup path, and no OCR-derived storage path.

## Privacy And Scope Confirmation

This maintenance readiness branch does not expand the capture surface from
`v0.1.2`.
...
- If publication is not approved, keep `v0.1.2` as the latest stable release.
...
- If a regression is found before publication and requires product code,
  ...
  capture-surface changes, stop the direct `v0.1.3` path and prepare a release
  candidate.
...
- Release path: compatible `v0.1.3` maintenance release, pending explicit
  publication approval.
- Fallback path: release candidate if any product or contract change is
  required.
- Deterministic gates: local M4 validation and PR Windows Harness passed;
  post-merge `main` Windows Harness is pending.
